...
- **左键图标**：弹出详情窗口（5 小时窗口、周额度、月总额度、Extra Usage 余额及重置时间），窗口可常驻、每次刷新后自动同步数据
- **右键菜单**：额度详情、立即刷新、显示源、刷新间隔（1/3/5/10 分钟）、低额度气泡提醒、开机自启、打开控制台
- **双击图标**：立即刷新
...
- 本工具使用自己的 User-Agent（`kimi-quota-tray/1.0`），不伪装官方客户端
...
产出单文件 `KimiQuotaTray.exe`。完整设计规格见 [计划书.md](计划书.md)。
